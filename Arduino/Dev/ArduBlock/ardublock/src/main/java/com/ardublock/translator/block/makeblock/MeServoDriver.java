	...
	public String toCode() throws Exception {
		...
		if(block instanceof NumberBlock){
			...
			if(deviceId>2||deviceId<1){
				throw new BlockException(this.getBlockId(), "the Device Id of Servo must be in Range(1,2)");
			}
		...
		if(block instanceof NumberBlock){
			...
			if(angle>180||angle<0){
				throw new BlockException(this.getBlockId(), "the angle of Servo must be in Range(0,180)");
			}
