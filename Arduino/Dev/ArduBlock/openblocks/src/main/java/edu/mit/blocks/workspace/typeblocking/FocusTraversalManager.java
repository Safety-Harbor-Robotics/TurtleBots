...
/**
 * The FocusTraversalManager has two function. First, it maintains a pointer to
 * the block, if any, that has focus and the corresponding focus point on that
 * block. If the focus is not on the block, then it must be set to some point of
 * the block canvas.
 * 
 * The second primary function of the FocusTraversalManager is to redirect the
 * focus to the next appropriate block in a particular stack. One could
 * "traverse" the stack by moving the focus to one of the following: 1. the
 * block after 2. the block before 3. the next block 4. the previous block
 * 
 * The exact definition of what "next", "previous", "after", and "before" is
 * described in details in their corresponding method summary. As a focus
 * manager of the entire system, the class must maintain particular invariants
 * at all time. Clients of this module may obtain the focus through three
 * observer (getter) methods. Clients may also manualy mutate the focus through
 * three modifier (setter) methods.
 * 
 * However, BOTH the value returned in the observer methods and the value passed
 * in the modifier methods MUST maintain particular invariants described below.
 * 
 * These invariants must hold at all time and check reps should be imposed to
 * ensure that any changes to the system still holds these crucial invariants.
 * Clients of this module may assume that the invariants mentioned below will
 * always hold.
 * 
 * INVARIANT I. If the canvas has focus, then the block does not. Thus 1.
 * focusBlock == Block.null 2. canvasFocusPoint != null 3. blockFocusPoint ==
 * null
 * 
 * INVARIANT II. If the block has focus, then the canvas does not. Thus 1.
 * focusBlock != Block.null 2. canvasFocusPoint == null 3. blockFocusPoint !=
 * null
 * 
 * @specfield focusBlock : Long //block with focus
 * @specfield canvasFocusPoint : Point //focus point on canvas relative to
 *            canvas
 * @specfield blockFocusPoint : Point //focus point on block relative to block
 * 
 */
public class FocusTraversalManager implements MouseListener, KeyListener,
		WorkspaceListener {
		...
		if (!invalidBlock(focusBlock)) {
			throw new RuntimeException(
					"May not request canvas's focus point if "
							+ "canvas does not have focus. Focus at: "
							+ focusBlock);
		}
		if (blockFocusPoint != null) {
			throw new RuntimeException(
		...
		if (invalidBlock(focusBlock)) {
			throw new RuntimeException(
					"May not request block's focus point if "
							+ "block does not have focus. Focus at: "
							+ focusBlock);
		}
		if (blockFocusPoint == null) {
			throw new RuntimeException(
	...
	public void setFocus(Long blockID) {
		if (blockID == null || blockID == Block.NULL || blockID == -1
				|| workspace.getEnv().getBlock(blockID) == null) {
			throw new RuntimeException("Invariant Violated:"
					+ "may not set focus to a null Block instance");
			// Please do not remove exception above. This class
			// and many other classes within the typeblocking
		...
		if (!invalidBlock(this.focusBlock)) {
			...
			workspace.getEnv().getRenderableBlock(this.focusBlock)
					.repaintBlock();
		}
		...
		// System.out.println("FocusManager: Setting focus to block: " +
		// this.focusBlock+", "+this.blockFocusPoint+", "+this.canvasFocusPoint);
	}
	...
	public void setFocus(Point canvasPoint, Long blockID) {
		if (blockID == null || blockID == Block.NULL || blockID == -1
			...
			// System.out.println("FocusManager: Setting focus to canvas: " +
			// this.focusBlock+", "+this.blockFocusPoint+", "+this.canvasFocusPoint);
		} else {
			throw new RuntimeException("Invariant Violated:"
	...
	void setFocus(Point location) {
		...
		// may not use this method as it does not ensure class invariant will
		// hold
	...
	// ////////////////////////////////////
	/**
	 * Reassigns the focus to the "next block" of the current focusBlock. If the
	 * current focusblock is at location n of the flatten linear vector of the
	 * block tree structure, then the "next block" is located at n+1. In other
	 * words, the previous block is the parent block of the next socket of the
	 * parent block of the focusblock.
	 * 
	 * @requires pointFocusOwner != null && focusblock.getSockets() != null &&
	 *           focusblock.getSockets() is not empty
	 * @modifies this.focusblock
	 * @effects this.focusblock now points to the "next block" as described in
	 *          method overview;
	...
	public boolean focusNextBlock() {
		...
		if (invalidBlock(focusBlock)
				|| !workspace.getEnv().getRenderableBlock(focusBlock)
						.isVisible()) {
		...
		if (nextBlock == null) {
			throw new RuntimeException(
					"Invariant Violated: return value of getNextNode() may not be null");
		}
		// set focus
	...
	/**
	 * Reassigns the focus to the "previous block" of the current focusBlock. If
	 * the current focusblock is at location n of the flatten linear vector of
	 * the block tree structure, then the "previous block" is located at n-1. In
	 * other words, the previous block is the innermost block of the previous
	 * socket of the parent block of the focusblock.
	 * 
	 * @requires pointFocusOwner != null && focusblock.getSockets() != null &&
	 *           focusblock.getSockets() is not empty
	 * @modifies this.focusblock
	 * @effects this.focusblock now points to the "previous block" as described
	 *          in method overview;
	 * @return true if the new focus is on a block that isn't null
	 */
	public boolean focusPrevBlock() {
		...
		if (previousBlock == null) {
			...
			// then get innermost block of the previous socket of the previous
			// block
			// assumes previousBlock.getSockets is not empty, not null
			Block beforeBlock = previousBlock;
			// ASSUMPTION BEING MADE: assume that the list below is constructed
			// to
			// have all the sockets FOLLOWED by FOLLOWED by the after connector
			// THE ORDER MUST BE KEPT TO WORK CORRECTLY! We cannot use
			// BlockLinkChecker.getSocketEquivalents because the specification
			// does
			// not guarantee this precise ordering. Futhermore, an interable
			// has no defined order. However, as of this writing, the current
			// implementation
			// of that method does seem to produce this ordering. But we're
			// still not using it.
			...
			connections.add(previousBlock.getAfterConnector()); // add after
																// connector
			// now traverse the connections
			for (BlockConnector connector : connections) {
				if (connector == null || connector.getBlockID() == Block.NULL
						|| getBlock(connector.getBlockID()) == null) {
					continue; // if null socket, move on to next socket
				}
				if (connector.getBlockID().equals(currentBlock.getBlockID())) { // reached
																				// back
																				// to
																				// current
																				// block
					if (!beforeBlock.getBlockID().equals(
							previousBlock.getBlockID())) {
						// if previous block was never updated, go to bottom of
						// stack
	...
	/**
	 * Gives focus to the first after block down the tree, that is, the next
	 * control block in the stack. If next control block does not exist, then
	 * give focus to current focusblock. Otherwise, give focus to block canvas.
	 * 
	 * Expects no wrapping to TopOfStack block, do not use this method for infix
	 * blocks
	 * 
	 * @requires focusblock.isMinimized() == false
	 * @modifies this.focusblock
	 * @effects sets this.focusblock to be the first after block if possible.
	 *          Otherwise, keep the focus on the current focusblock. If focus
	 *          block is an invalid block, return focus to the default (block
	 *          canvas)
	...
	/**
	 * Gives focus to the first beforeblock up the tree, that is, the previous
	 * control block in the stack. If no previous control block exists, then
	 * give focus to current focusblock. Otherwise, give focus to block canvas.
	 * 
	 * Expects no wrapping to bottom block, do not use this method for infix
	 * blocks
	 * 
	 * @requires focusblock.isMinimized() == false
	 * @modifies this.focusblock
	 * @effects sets this.focusblock to be the first before block if possible.
	 *          Otherwise, keep the focus on the current focusblock. If focus
	 *          block is an invalid block, return focus to the default (block
	 *          canvas)
	 ...
	 * @param currentBlock
	 * @return currentBlock or NON-NULL block that is the next node of
	 *         currentBlock
	 */
	private Block getNextNode(Block currentBlock) {
		...
		if (invalidBlock(currentBlock)) {
			throw new RuntimeException(
					"Invariant Violated: may not resurve over a null instance of currentBlock");
		}
		// if plug not null, then let plug be parent of current block
		Block parentBlock = getBlock(currentBlock.getPlugBlockID());
		// otherwise if after not null, then let after be parent of current
		// block
		...
		int i = parentBlock.getSocketIndex(parentBlock
				.getConnectorTo(currentBlock.getBlockID()));
		...
		if (i != -1 && i >= 0) {
			for (BlockConnector parentSocket : parentBlock.getSockets()) {
				if (parentSocket == null
						|| invalidBlock(parentSocket.getBlockID())
						|| parentBlock.getSocketIndex(parentSocket) <= i) {
					continue;
				} else {
	...
	/**
	 * For a given block, returns the outermost (top-leftmost) block in the
	 * stack.
	 * 
	 * @requires block represented by blockID != null
	 * @param blockID
	 *            any block in a stack.
	 * @return the outermost block (or Top-of-Stack) such that the outermost
	 *         block != null
	 */
	Long getTopOfStack(Long blockID) {
		...
		// check invariant
		// TODO: Remove the "invariant" below because it's covered in the
		// conditional above!
		// if (parentBlock != null) {
	...
	/**
	 * For a given block, returns the innermost (bottom-rightmost) block in the
	 * substack.
	 * 
	 * @requires block !=null block.getBlockID != Block.NULL
	 * @param block
	 *            the top block of the substack.
	 * @return the innermost block in the substack. such that the innermost
	 *         block != null
	 */
	private Block getBottomRightBlock(Block block) {
		...
		for (BlockConnector socket : block.getSockets()) {// assumes
															// socket!=null
	...
	/**
	 * @param block
	 * @return true if and only if block ==null || block.getBlockID == null &&
	...
	/**
	 * All the private methods below follow a similar specification. They all
	 * require that the block referanced by blockID (or block.getBlockID) is
	 * non-null. If getting a socket block, they additionally require that
	 * 0<socket< # of sockets in block. All the methods before return a block
	 * located at a block connector corresponding to the name of the obserser
	 * method.
	 * 
	 * @requires blockID != Block.Null && block !=null
	 * @return Block instance located at corresponding connection or null if non
	 *         exists
	 */
	...
	// /////////////////
	/**
	 * Action: removes the focus current focused block and places new focus on
	 * e.getSource
	 * 
	 * @requires e != null
	 * @modifies this.blockFocusOwner && e.getSource
	 * @effects removes focus from this.blockFocusOwner adds focus to
	 *          e.getSource iff e.getSource is instance of BlockCanvas and
	 *          RenderableBlock
	 */
	private void grabFocus(MouseEvent e) {
		// System.out.println("FocusManager: Mouse Event at ("+
		// e.getX()+", "+e.getY()+") on "+e.getSource());
		if (e.getSource() instanceof Canvas) {
			// get canvas point
			Point canvasPoint = e.getPoint();
			/*
			 * SwingUtilities.convertPoint( (BlockCanvas)e.getSource(),
			 * e.getPoint(), ((BlockCanvas)e.getSource()).getCanvas());
			 */
	...
	// /////////////////////////////
	/**
	 * Subscription: BLOCK_ADDED events. Action: add this.mouselistener to the
	 * block referanced by event
	 * 
	 * @requires block reference in event is not null
	 * @modifies this.blockFocusOwner && event.block
	 * @effects Add this.mouselistener to this.blockFocusOwner removes focus
	 *          from this.blockFocusOwner adds focus to e.getSource iff
	 *          e.getSource is instance of BlockCanvas and RenderableBlock
	 */
	public void workspaceEventOccurred(WorkspaceEvent event) {
		...
		case WorkspaceEvent.BLOCK_ADDED:
			...
			RenderableBlock rb = workspace.getEnv().getRenderableBlock(
					event.getSourceBlockID());
			if (rb == null) {
				break;
			}
	...
	public String toString() {
		return "FocusManager: " + blockFocusPoint + " of "
				+ workspace.getEnv().getBlock(focusBlock);
	}
}
